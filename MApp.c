...
static uint8_t count = 0;
		...
		struct NODES{
			uint8_t Short_Address[5];
			uint8_t Extended_Address[5];
			uint8_t RxOnWhenIdle[5];
...
static uint8_t App_SendAssociateResponse(nwkMessage_t *pMsgIn)
{
  ...
  if(pMsg != NULL)
  {
    ...
    for(count = 0;count < 5; count++){
    	...
    	if(FLib_MemCmp((uint8_t *)node.Extended_Address[count],pMsgIn->msgData.associateInd.deviceAddress,8)){
    		FLib_MemCpy((uint8_t *)node.Short_Address[count],(uint8_t *)(count),2);
    		pAssocRes->assocShortAddress[0] = 0x00;
    		pAssocRes->assocShortAddress[1] = count;
    		break;
    	}else if (node.Extended_Address[count] == 0){
    		FLib_MemCpy((uint8_t *)node.Extended_Address[count],pMsgIn->msgData.associateInd.deviceAddress,8);    		
    		FLib_MemCpy((uint8_t *)node.Short_Address[count],(uint8_t *)(count),2);    		
    		pAssocRes->assocShortAddress[0] = 0x00;
    		pAssocRes->assocShortAddress[1] = count;
    		break;
